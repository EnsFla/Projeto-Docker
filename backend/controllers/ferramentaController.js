...
exports.listarParaAlmoxarife = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("SELECT * FROM solicitacoes_ferramentas WHERE tecnico_id = $1", [usuario_id]);
    res.json(result.rows);
  } catch (error) {
    console.error("Erro ao buscar solicitações:", error);
    res.status(500).json({ error: "Erro no servidor" });
...
exports.aceitar = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE solicitacoes_ferramentas SET status = 'aceita', tecnico_id = $2 WHERE id = $1 RETURNING *",
      [id, user_id]
    );
