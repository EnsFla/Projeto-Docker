...
const TelaAlmoxarifado = () => {
  ...
  const buscarSolicitacoes = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/ferramentas/almoxarife/${almoxarife_id}`);
  ...
  const atualizarSolicitacao = async (solicitacaoId, status) => {
    try {
      const url = status === "aceita"
        ? `http://127.0.0.1:5000/ferramentas/${solicitacaoId}/aceitar/${almoxarife_id}`
        : status === "rejeitada"
        ? `http://127.0.0.1:5000/ferramentas/${solicitacaoId}/rejeitar`
        : status === "concluida"
        ? `http://127.0.0.1:5000/ferramentas/${solicitacaoId}/concluir`
        : null;
