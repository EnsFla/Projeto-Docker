...
const TelaTecnico = () => {
  ...
  const buscarSolicitacoes = async () => {
    try {
      ...
      const responsePendentesManutencao = await fetch(`http://127.0.0.1:5000/manutencao/tecnico/${tecnico_id}`);
      const dataPendentesManutencao = await responsePendentesManutencao.json();

      const responseAceitasManutencao = await fetch(`http://127.0.0.1:5000/manutencao/tecnico/${tecnico_id}/aceitas`);
  ...
  const aceitarSolicitacaoManutencao = async (solicitacaoId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/manutencao/${solicitacaoId}/aceitar`, {
        method: "PUT",
        headers: {
  ...
  const concluirSolicitacaoManutencao = async (solicitacaoId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/manutencao/${solicitacaoId}/concluir`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  try {
    const response = await fetch("http://127.0.0.1:5000/ferramentas", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    // Tentar interpretar como JSON, mas proteger contra resposta não JSON
    ...
    if (response.ok) {
      ...
      setSolicitacaoFerramenta(""); // Limpar campo após sucesso
    } else {
      setMessage(data?.error || "Erro ao solicitar ferramenta");
    }
  } catch (error) {
    console.error("Erro na requisição:", error);
    setMessage("Erro ao solicitar ferramenta");
  }
};
